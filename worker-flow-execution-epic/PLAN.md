...
| [`TASK-A6-FRONT-DRIVEN-NODES.md`](./TASK-A6-FRONT-DRIVEN-NODES.md) | Give the eight front-driven types server-side execution for the first time (nine in the census; `imageReaderNode` deprecated — D24) | A1 |
...
| D3 | The front-driven types (eight, after D24) do not run in headless flows today. Intended, or a silent defect for customers with one in a scheduled flow? | A6 | Independent of the worker. Answer it on its own before A6 turns it into a behaviour change. **Partial input, 2026-09-02, awaiting product confirmation:** `documentSummarizer` and `commandMusicNode` are discontinued; `webAmazon` and `secApiNode` are broken today; `fileSave` is under review. If confirmed, A6's Stage 2 loses two of three and Stage 3 loses one — record the confirmation here before Stage 1. |
