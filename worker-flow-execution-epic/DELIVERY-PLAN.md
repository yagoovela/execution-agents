...
How the 28 tasks in this epic become shipped changes, in an order where **each wave is useful on
...
Today the system is protected by three accidents: one sequential engine loop, one queued run per
backend replica, and a blocking wait on every Temporal node. None of them was designed as a
safeguard, and all three disappear in this epic.
...
| `S7` part 1 | An unauthenticated webhook route that runs **any** flow and charges its owner |
...
| `S3` | Parallel dispatch later, by bounding spend and bounding one tenant's share |
...
| `A5` | The image family, over one shared provider layer |
| `A6` + `S6` | The nine front-driven types. **S6's egress decision ships with them, not after** |
...
**Observable after.** Flows that silently produced stale output in headless runs — the nine
...
test green. Cancellation works through a child workflow.
...
| `B6` | Control flow and sub-flows as workflow constructs and child workflows |
| `B7` | CSV batches as durable workflows — the largest multiplier of every ceiling |
...
**Runs in parallel with it, no contention:** S4, S5, C3 (Wave 0); S1 and S3 (Wave 1, but both must
land before their gates — S1 before B6, S3 before B5); E3 (before B5); D1 and D2 (continuous);
C1 (attached to each node's task).
...
| 13 | Leader election for the fourteen crons | `S8` | 1 |
