...
GLANCE = [
 ('crit', ('Severity', 'Severidade'), ('High', 'Alta'),
  ('Fourteen crons with no leader election, and schedules held in an in-process registry. Review §9.3, §9.4, §9.5.',
   'Catorze crons sem eleição de líder, e agendamentos guardados num registro em processo. Review §9.3, §9.4, §9.5.')),
...
LEDE = (
 """<p><code>@Cron</code> is registered <strong>fourteen times</strong> across <code>back/src</code>, and there is no leader election: greps for <code>leader</code>, <code>isLeader</code>, <code>CRON_ENABLED</code> or an advisory-lock guard return nothing. <strong>Every cron fires on every replica.</strong></p>
<p>User schedules are worse, because they cost money. They are registered in NestJS&#x27;s in-process registry — <code>schedulerRegistry.addCronJob</code> at <code>schedule.controller.ts:179</code> and <code>:332</code> — so each replica holds its own copy and fires it independently. <strong>N replicas means N runs of the same schedule, each charged to the customer.</strong></p>""",
 """<p>O <code>@Cron</code> está registrado <strong>catorze vezes</strong> em <code>back/src</code>, e não há eleição de líder: buscas por <code>leader</code>, <code>isLeader</code>, <code>CRON_ENABLED</code> ou uma guarda de advisory lock não retornam nada. <strong>Todo cron dispara em toda réplica.</strong></p>
<p>Os agendamentos de usuário são piores, porque custam dinheiro. Eles são registrados no registro em processo do NestJS — <code>schedulerRegistry.addCronJob</code> em <code>schedule.controller.ts:179</code> e <code>:332</code> — então cada réplica guarda sua própria cópia e dispara sozinha. <strong>N réplicas significam N runs do mesmo agendamento, cada um cobrado do cliente.</strong></p>""")
...
TABLE = {
 ...
 'rows': [
  [{'t': ('Fourteen <code>@Cron</code> sites', 'Catorze pontos com <code>@Cron</code>')},
  ...
  [{'t': ('User schedules', 'Agendamentos de usuário')},
   ('<code>schedule.controller.ts:179</code>, <code>:332</code>', '<code>schedule.controller.ts:179</code>, <code>:332</code>'),
  ...
  [{'t': ('The POP3 poll', 'O poll POP3')},
   ('<code>mail.service.ts:132</code>', '<code>mail.service.ts:132</code>'),
  ...
  [{'t': ('<code>markStuckSpaceRunLogs</code>', '<code>markStuckSpaceRunLogs</code>')},
   ('One of the fourteen', 'Um dos catorze'),
  ...
  [{'t': ('Redis dedup keys', 'Chaves de dedup no Redis')},
   ('<code>flux.service.ts:5030–5044</code>', '<code>flux.service.ts:5030–5044</code>'),
...
DEC_CRON = {
 ...
 'q': ('What makes a cron fire once, when fourteen of them fire on every replica?',
       'O que faz um cron disparar uma vez, quando catorze deles disparam em toda réplica?'),
 'intro': (
  ...
  'The requirement is <strong>one mechanism for all fourteen, not fourteen guards</strong> — and the property worth optimising for is that '
  ...
  'O requisito é <strong>um mecanismo para os catorze, não catorze guardas</strong> — e a propriedade a otimizar é que '
 ...
 'opts': [
  {'ltr': 'A', 'pick': True, 'name': ('An advisory lock per job name', 'Um advisory lock por nome de job'),
   ...
   'pros': [('<strong>Nothing has to know the replica count</strong> — the mechanism is correct at one replica and at ten',
            ...
            ('<code>pg_advisory_xact_lock</code> is already used in the codebase (<code>oauth-token.repo.ts:7</code>), so there is no new dependency',
             'O <code>pg_advisory_xact_lock</code> já é usado no código (<code>oauth-token.repo.ts:7</code>), então não há dependência nova'),
            ...
            ('One mechanism covers all fourteen, including the POP3 poll in Part 4',
             'Um mecanismo cobre os catorze, incluindo o poll POP3 da Parte 4')],
   'cons': [('It puts a lock acquisition on the database this epic is already worried about — fourteen jobs, one of them every ten seconds',
             'Coloca uma aquisição de lock no banco com que este épico já se preocupa — catorze jobs, um deles a cada dez segundos'),
  ...
  {'ltr': 'C', 'name': ('Leader election', 'Eleição de líder'),
   ...
   'cons': [('It is a distributed-systems component to run and reason about, for fourteen maintenance jobs',
             'É um componente de sistemas distribuídos para rodar e raciocinar, por causa de catorze jobs de manutenção'),
  ...
  {'ltr': 'D', 'name': ('Temporal Schedules for all fourteen', 'Temporal Schedules para os catorze'),
   ...
   'cons': [('<strong>These fourteen are internal maintenance and have no reason to become workflows</strong>',
             '<strong>Estes catorze são manutenção interna e não têm motivo para virar workflows</strong>'),
   ...
   'cost': [('lo', ('Client effort: <b>none</b>', 'Esforço do cliente: <b>nenhum</b>')),
            ('hi', ('Ours: <b>fourteen jobs restructured for no user-facing gain</b>', 'Nosso: <b>catorze jobs reestruturados sem ganho para o cliente</b>'))]},
 ],
 'rec': (
  '<p><strong>A for the fourteen framework crons, D for the user-facing schedules in Part 2.</strong> That split is the review&#x27;s amendment (§11.3), and it is deliberate: the crons are internal maintenance and Temporal Schedules buy them nothing they need, while the user schedules are customer-visible, durable, and want pause, backfill and last-run visibility an in-process registry cannot offer.</p>'
...
DEC_REDIS = {
 ...
 'intro': (
  ...
  'The dedup pattern itself is correct — <code>SET key &#x27;1&#x27; EX 86400 NX</code>, run-scoped (<code>flux.service.ts:5030–5044</code>) — and it is what stops a retried run from emailing a customer twice. '
  ...
  'O padrão de dedup em si está correto — <code>SET key &#x27;1&#x27; EX 86400 NX</code>, com escopo de run (<code>flux.service.ts:5030–5044</code>) — e é o que impede um run com retry de mandar e-mail duas vezes para o cliente. '
...
PARTS = [
{'n': '1',
 'title': ('The fourteen crons', 'Os catorze crons'),
 'loc': 'back/src/cronJobs/** · oauth-token.repo.ts:7',
 'purpose': ('One mechanism for all fourteen, so a second replica stops multiplying maintenance work.',
             'Um mecanismo para os catorze, para que uma segunda réplica pare de multiplicar trabalho de manutenção.'),
 'body': (
  '<p><code>@Cron</code> is registered fourteen times across <code>back/src</code>, and there is no leader election. Every cron fires on every replica.</p>'
  ...
  '<p>O <code>@Cron</code> está registrado catorze vezes em <code>back/src</code>, e não há eleição de líder. Todo cron dispara em toda réplica.</p>'
...
{'n': '2',
 ...
 'loc': 'schedule.controller.ts:157–159, 179, 242, 283–297, 309–311, 332',
 ...
 'body': (
  '<p>Scheduled runs do not use a durable scheduler. They are registered in NestJS&#x27;s in-process registry — <code>schedulerRegistry.addCronJob</code> at <code>:179</code> and <code>:332</code> — so each replica holds its own copy and fires it independently. <strong>N replicas means N runs of the same schedule, each charged to the customer.</strong></p>'
  '<p>Two defects live in the same code, and both are fixed here:</p>',
  '<p>Runs agendados não usam um scheduler durável. Eles são registrados no registro em processo do NestJS — <code>schedulerRegistry.addCronJob</code> em <code>:179</code> e <code>:332</code> — então cada réplica guarda a própria cópia e dispara sozinha. <strong>N réplicas significam N runs do mesmo agendamento, cada um cobrado do cliente.</strong></p>'
  '<p>Dois defeitos vivem no mesmo código, e os dois são corrigidos aqui:</p>'),
 'list': [
  ('<strong>The re-registration hides in a getter.</strong> <code>getAllSchedules()</code> (<code>:242</code>) is a plain method with no route decorator that walks every active schedule and registers crons <em>as a side effect</em>. Whatever calls it registers crons on that replica.',
   '<strong>O re-registro se esconde num getter.</strong> O <code>getAllSchedules()</code> (<code>:242</code>) é um método comum, sem decorator de rota, que percorre todos os agendamentos ativos e registra crons <em>como efeito colateral</em>. Quem quer que o chame registra crons naquela réplica.'),
  ('<strong>The queue is not doing the work.</strong> The callback does <code>await this.scheduleQueue.add(&#x27;schedule-job&#x27;, { … job: await this.fluxService.apiV2({ … }) })</code> (<code>:157–159</code>, <code>:309–311</code>) — the run is <strong>awaited as an argument to the enqueue call</strong>, so the flow already executed synchronously inside the cron callback and only its <em>result</em> reaches the queue.',
   '<strong>A fila não está fazendo o trabalho.</strong> O callback faz <code>await this.scheduleQueue.add(&#x27;schedule-job&#x27;, { … job: await this.fluxService.apiV2({ … }) })</code> (<code>:157–159</code>, <code>:309–311</code>) — o run é <strong>aguardado como argumento da chamada de enfileiramento</strong>, então o fluxo já executou de forma síncrona dentro do callback do cron e só o <em>resultado</em> dele chega na fila.'),
 ],
 'body2': (
  ...
  '<p><strong>Keep the zombie-cron check</strong> at <code>:283–297</code> — stop, delete, soft-delete when the flow is gone. It is the one part of this code already doing the right thing.</p>',
  ...
  '<p><strong>Mantenha a checagem de cron zumbi</strong> em <code>:283–297</code> — parar, apagar, apagar logicamente quando o fluxo já não existe. É a única parte deste código que já faz a coisa certa.</p>'),
 ...
 'callouts': [('decide', ('Why Temporal Schedules here and a lock there', 'Por que Temporal Schedules aqui e um lock lá'),
   ('<p>Greps for <code>ScheduleClient</code>, <code>scheduleClient</code> and <code>createSchedule</code> across the worker and <code>back/src/temporal</code> return nothing, so the platform&#x27;s native scheduling is entirely unused. It gives by construction exactly what this part would otherwise build by hand.</p>'
    '<p>The advisory lock stays the right answer for the <strong>fourteen framework crons</strong>, which are internal maintenance and have no reason to become workflows (review §11.3).</p>',
    ...
    '<p>O advisory lock continua sendo a resposta certa para os <strong>catorze crons de framework</strong>, que são manutenção interna e não têm motivo para virar workflows (review §11.3).</p>'))]},
{'n': '3',
 'title': ('Redis carries three unrelated responsibilities', 'O Redis carrega três responsabilidades sem relação'),
 'loc': 'flux.service.ts:5030–5044 · Redis/infra',
...
{'n': '4',
 ...
 'body': (
  '<p>The email poll is one of the fourteen crons, so it fires every ten seconds <strong>per replica</strong> (<code>:132</code>). There is no message-id dedup and no lock — greps for <code>messageId</code>, <code>dedup</code>, <code>lock</code> and <code>NX</code> return nothing. Messages are fetched, processed and only then deleted (<code>deleteEmailsSequentially</code>, <code>:314</code>), so the window between fetch and delete spans the whole batch, <strong>including enqueueing the runs</strong>.</p>'
  ...
  '<p>O poll de e-mail é um dos catorze crons, então dispara a cada dez segundos <strong>por réplica</strong> (<code>:132</code>). Não há dedup por id de mensagem nem lock — buscas por <code>messageId</code>, <code>dedup</code>, <code>lock</code> e <code>NX</code> não retornam nada. As mensagens são buscadas, processadas e só então apagadas (<code>deleteEmailsSequentially</code>, <code>:314</code>), então a janela entre buscar e apagar cobre o lote inteiro, <strong>incluindo o enfileiramento dos runs</strong>.</p>'
 ...
 'body2': (
  '<p><strong>Two independent guards</strong>, because the failure they prevent is a charge to a customer: Part 1&#x27;s advisory lock covers the polling, and — independently of it — dedup by message id before enqueueing, using the same <code>SET … NX</code> pattern already used for delivery (<code>flux.service.ts:5030–5044</code>).</p>'
  ...
  '<p><strong>Duas guardas independentes</strong>, porque a falha que elas evitam é uma cobrança ao cliente: o advisory lock da Parte 1 cobre o polling e — independentemente dele — dedup por id de mensagem antes de enfileirar, usando o mesmo padrão <code>SET … NX</code> já usado na entrega (<code>flux.service.ts:5030–5044</code>).</p>'
...
FILES = [
 ('back/src/cronJobs/** (fourteen @Cron sites)', False),
 ('back/src/app-api/mail/mail.service.ts:132, 314', False),
 ('back/src/app-api/schedule/schedule.controller.ts:157–159, 179, 242, 283–297, 309–311, 332', False),
 ('back/src/app-api/flux/flux.service.ts:5030–5044', False),
